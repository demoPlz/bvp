        ...
        if self.node.initialized:
            ...
            if self.mirror_operator_view:
                ...
                if not self.disable_orientation_control:
                    ...
                    axis_angle = rotations.axis_angle_from_matrix(ee_rot)
                    axis = axis_angle[:3]
                    angle = axis_angle[3]
                    axis_norm = np.linalg.norm(axis)
                    if axis_norm > 1e-8:
                        axis = axis / axis_norm
                        axis = FRONT_FACING_AXIS_MIRROR @ axis
                        mirrored_quat = rotations.quaternion_from_axis_angle(
                            np.concatenate([axis, [angle]])
                        )
                    else:
                        mirrored_quat = ee_quat.copy()

                    mirrored_rot = rotations.matrix_from_quaternion(mirrored_quat)
